...
const WEAPON_PROPERTIES = {
  ...
  agile:        { type: "flavor" },
  ammunition:   { type: "flavor", native: true }, // dnd5e 4.x already has this natively
  ...
  finesse:      { type: "flavor", native: true }, // dnd5e 4.x already has this natively
  flense:       { type: "oncrit", condition: "targetUnarmored" },
  flexible:     { type: "special", valued: true }, // Flexible (X)
  flourish:     { type: "flavor" },
  heavy:        { type: "flavor", native: true }, // dnd5e 4.x already has this natively
  hidden:       { type: "flavor" },
  lacerate:     { type: "special", valued: true }, // Lacerate (X) — Stage 2
  light:        { type: "flavor", native: true }, // dnd5e 4.x already has this natively
  liquidate:    { type: "oncrit", condition: "targetSubmerged" },
  loading:      { type: "flavor", native: true }, // dnd5e 4.x already has this natively
  ...
  reach:        { type: "flavor", native: true }, // dnd5e 4.x already has this natively
  reload:       { type: "special", valued: true, native: true }, // dnd5e 4.x has this natively, including ammo tracking
  repeater:     { type: "flavor" },
  special:      { type: "flavor" }, // marks a weapon with unique rules text (see item description)
  swift:        { type: "flavor" },
  thrown:       { type: "flavor", native: true }, // dnd5e 4.x already has this natively
  trip:         { type: "flavor" },
  twoHanded:    { type: "flavor", native: true }, // dnd5e 4.x already has this natively
  unwieldy:     { type: "flavor" },
  versatile:    { type: "flavor", native: true } // dnd5e 4.x already has this natively
};
...
function registerWeaponProperties() {
  for (const [key, data] of Object.entries(WEAPON_PROPERTIES)) {
    // Skip anything already built into this version of dnd5e (see the
    // `native: true` flags above) — these are standard 5e weapon
    // properties, not homebrew-specific ones, so we defer to the system's
    // own checkbox/behavior instead of creating a second, non-functional
    // duplicate with the same label.
    if (data.native) continue;
